...
std::shared_ptr<s21::NetworkService> ns_ =
    std::make_shared<s21::NetworkService>();
s21::NetworkService::neural_ptr nn_;

void setCurrentValues(s21::Metrics) {}
std::function<void(s21::Metrics)> statistic_ptr = &setCurrentValues;

TEST(graph, test_1) {
  nn_ = std::make_shared<s21::GraphNetwork>(num_hidden_layers);
...
TEST(matrix, test_1) {
  nn_ = std::make_shared<s21::MatrixNetwork>(num_hidden_layers);
  ns_->SetNeuralNetwork(nn_);
  ns_->LoadDataset(dataset_file);
  ns_->SetStatisticFunc(statistic_ptr);
  ns_->Train(1);
